...
Tfは特定の「ドキュメント」内の「用語」の頻度
速い茶色のキツネが怠惰な犬を飛び越えます。なんてキツネ！
速い茶色のキツネが怠惰なキツネを飛び越えます。なんてキツネ！
の場合
キツネという用語は上は2/12 下は3/12
idfはlog(全ドキュメント数/Xを含むドキュメント数)なので
上も下も0
tf-idfは(2/12)*0=0 (3/12)*0=0
で両方0

ロジステック回帰で感情分析できんことない

# 前処理
## 特徴量
### 周期的特徴量の生成
周期的特徴を表現する最良の方法は、sin関数とcos関数を使用して、これらを円上の（x、y）座標として表すと良い
時間帯とか曜日とか月とか
...
## 過学習の場合
- 特徴量の相関をみて選択する特徴量を減らす

# 検証
## ホールドアウト検証
あらかじめ学習データと検証データを分けるやつ
## 交差検証
用意したデータをK個に分割して、1回目はそのうちの一つをテストデータ、それ以外を学習データとして、学習・評価します。2回目は1回目と異なる別のデータをテストデータとして使い、3回目は1,2回目と異なるデータで評価
...
## 典型的な前処理
### 単語の分割（トークン化）
日本語は文章にスペースがないので文章から単語を分割する
### ストップワードの削除
助詞や助動詞などの出現頻度が高いが意味がない機能語(「は」「の」「です」「ます」など)を削除する
### 単語の正規化
#### 文字種の統一
アルファベットの大文字を小文字に変換する
半角文字を全角文字に変換
数字の置き換え（数字を全部0に置き換える）
単語の統一（同じ意味のやつを同じ言葉に置き換える　りんご　林檎　リンゴ）

# Word2Vec
単語を線形空間に写す
 "強い"や"強固", "頑丈"等の単語は空間上で近くに存在し, 逆に"弱小"や"弱い"等のの単語は遠くに存在する
## N-gram
これは文字列や単語の列に対する概念で, 形式的には隣り合う単語で構成される長さNの部分列の集合
2-gramの場合
'私', 'は'
'は', 'お腹'
'お腹', 'が'
